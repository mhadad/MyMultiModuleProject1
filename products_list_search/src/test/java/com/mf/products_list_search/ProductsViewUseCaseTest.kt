package com.mf.products_list_search
...
class ProductsViewUseCaseTest {
    private lateinit var  remoteProductsRepo: RemoteProductsRepo
    private lateinit var productsUiModelMapper: ProductsUiModelMapper
    private lateinit var productsViewUseCase: ProductsViewUseCase
    private lateinit var retrofitMock: MockWebServer
    private lateinit var apiService: RemoteProductsDataSource
    ...
    @Before
    fun init(){
        retrofitMock = MockWebServer()
        apiService = Retrofit.Builder().baseUrl(retrofitMock.url("https://dummyjson.com")).addConverterFactory(
            GsonConverterFactory.create()).build().create(RemoteProductsDataSource::class.java)
        remoteProductsRepo = RemoteProductsRepo(apiService, null)
        productsUiModelMapper = mockk()
        productsViewUseCase = ProductsViewUseCase(remoteProductsRepo, productsUiModelMapper)
    }

    @Test
    fun `test GetAll Data Flow for 50 items limit, should return false`()= runBlocking{
        retrofitMock.enqueue(MockResponse().setBody(mockResponse).setResponseCode(200))
        val expectedProducts: ProductsDataModel? = Gson().fromJson(
            mockResponse,
            object : TypeToken<ProductsDataModel?>() {}.type
        )

        val actualProducts = remoteProductsRepo.getAllProducts(limit = 50)
        val isEqual = expectedProducts?.products?.count() == actualProducts?.products?.count()
        assertEquals(isEqual, false)
    }

    @Test
    fun `test GetAll Data Flow for 10 items limit, should return true`()= runBlocking{
        retrofitMock.enqueue(MockResponse().setBody(mockResponse).setResponseCode(200))
        val expectedProducts: ProductsDataModel? = Gson().fromJson(
            mockResponse,
            object : TypeToken<ProductsDataModel?>() {}.type
        ...
        val actualProducts = remoteProductsRepo.getAllProducts(limit = 10)
        val isEqual = expectedProducts?.products?.count() == actualProducts?.products?.count()
